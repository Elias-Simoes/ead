...
async function testCertificateValidation() {
  ...
  try {
    ...
    try {
      await axios.post(
        `${API_URL}/certificates/issue/${courseId}`,
        { courseId },
        { headers: { Authorization: `Bearer ${studentToken}` } }
      );
      console.log('   ❌ ERRO: Certificado foi emitido sem completar avaliações!');
    } catch (error) {
      if (error.response?.data?.error?.code?.includes('ASSESSMENTS_NOT_COMPLETED')) {
        console.log('   ✅ Certificado bloqueado corretamente!');
        console.log('   📝 Motivo:', error.response.data.error.code);
      } else {
    ...
    try {
      ...
      if (error.response?.data?.error?.code?.includes('FINAL_GRADE_BELOW_PASSING_SCORE')) {
        console.log('   ✅ Certificado bloqueado corretamente!');
        console.log('   📝 Motivo:', error.response.data.error.code);
      } else {
        console.log('   ❌ Erro inesperado:', error.response?.data || error.message);
